...
class acf_field_button extends acf_field
{
	...
	*/
	...
	function create_options( $field )
	{
		...
		<tr class="field_option field_option_<?php echo $this->name; ?>">
			<td class="label">
				<label><?php _e("Allow Style Editing?",'acf'); ?></label>
				<p class="description"><?php _e("Best to leave unchecked for major site theme elements, checked for free-form areas",'acf'); ?></p>
			</td>
			<td>
				<?php
				...
				do_action('acf/create_field', array(
					'type'		=>	'true_false',
					'name'		=>	'fields['.$key.'][allow_edit_style]',
					'value'		=>	$field['allow_edit_style'],					
					'default_value' => 0
				));
	...
	*/
	...
	function create_field( $field )
	{
		...
		<table class="acf_button">
			<tr>
				<?php if ($field['allow_edit_style']) { ?>
				<td>
					<label>Button Style</label>
					<?php
					do_action('acf/create_field', array(
						'type'		=>	'radio',
						'name'		=>	$field_name.'[btn_style]',
						'value'		=>	$field['value']['btn_style'],
						'layout'	=>	'horizontal',
						'choices'	=>	array(
							'btn-primary' => __('Primary Button'),
							'btn-default' => __('Default Button'),
							'btn-success' => __('Success Button'),
							'btn-info' => __('Info Button'),
							'btn-warning' => __('Warning Button'),
							'btn-danger' => __('Danger Button'),
							'btn-link' => __('Link Button'),
						)
					));?>
				</td>			
				<?php } //allow_edit_style check ?>
				<td valign="top">
					<label>Text</label>
	...
	*/
	...
	function format_value_for_api( $value, $post_id, $field )
	{
		...
		$value = '<a class="btn '.$button['btn_style'].'" href="' . $link .'"' . $target . '>' . $button['text'] . '</a>';
